...
function searchCubes(search, difficultyFrom, difficultyTo) {
    ...
    return getCubes()
        .then(cubes => {
            ...
            return { cubes: filteredCubes, search: search };
        });
}
...
function validateCube(cube) {
    ...
    let integerDifficulty = parseInt(cube.difficultyLevel); // Number.NaN
    if (integerDifficulty && (integerDifficulty <= 0 || integerDifficulty > 6)) {
        throw new TypeError('Difficulty is required and must be an integer value between 1 and 6.');
    }
}
